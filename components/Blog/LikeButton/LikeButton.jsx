...
const LikeButton = () => {
  ...
  return (
    <div className="button-container">
      <button
        onClick={() => {
          fetch("https://bapi.khairi.dev/like/" + url.split("/blog/").at(-1)).then(res => res.json()).then(data => {setSpam(data.spam)});
          if (!liked && spam) {
            showNotification({
              id: 'hello-there',
              disallowClose: true,
          ...
          if (liked) {
            fetch("https://bapi.khairi.dev/likedecent/unlike/" + url.split("/blog/").at(-1));
          } else {
            fetch("https://bapi.khairi.dev/likedecent/like/" + url.split("/blog/").at(-1));
          }
